...
import { blue, bold, cyan, gray, green, italic } from "@kitql/helpers";

const { version } = JSON.parse(
  fs.readFileSync(new URL("package.json", import.meta.url), "utf-8")
);
// eslint-disable-next-line no-console
console.log(`${gray(`create-remult version ${version}`)}\n`);
...
p.intro(`${blue(`⚡️`)} Welcome to ${blue(`remult`)} world!`);
...
copy(
  ...
  projectDir,
  {
    ...
    [`create-remult-${template}`]: projectName,
...
function copy(
  ...
  for (const fileSource of files) {
    const fileDest = Object.entries(transformFileMap).reduce(
      (acc, [key, value]) => {
...
function sourcePath(/** @type {string} */ path) {
  try {
    ...
    pCancel(
      "Not working in this environment. Are you on Windows? Have you look into WSL?"
    );
  }
}
